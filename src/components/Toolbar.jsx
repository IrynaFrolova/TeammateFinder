...
export default function Toolbar({
  ...
  // ... ваша існуюча логіка (хуки useState, useEffect) залишається без змін ...
  const [order, setOrder] = useState(() => {
    try {
      return (
        JSON.parse(localStorage.getItem("quickTagsOrder") || "null") ||
        dict.quickTags
      );
    } catch {
      return dict.quickTags;
    }
  });
  useEffect(() => {
    localStorage.setItem("quickTagsOrder", JSON.stringify(order));
  }, [order]);
  useEffect(() => {
  ...
  const onSel = (key) => (e) =>
    setFlt((prev) => ({ ...prev, [key]: e.target.value }));

  return (
    <section className={`toolbar ${className || ""}`}>
      {/* ===== ПОЧАТОК ЗМІН ===== */}
      <div className="toolbar__header">
        <h3>Filters</h3>
        <button
          className="btn btn--icon btn-close-toolbar"
          onClick={onClose}
          aria-label="Close filters"
        >
          &times; {/* Це іконка "хрестик" */}
        </button>
      </div>
      {/* ===== КІНЕЦЬ ЗМІН ===== */}
